...
#GWAS SNPs: Effect size distribution and SNP membership by trait class

#load in genome wide significant SNPs (one row per SNP-trait associations)
snps <- read_csv("SignificantGenomewideSNPs_withEffects.csv", show_col_types = FALSE)
#define trait-class mapping
metabolic_traits <- c(
...
# UpSet (UNIQUE SNPs membership by class)
...
p_upset <- ComplexUpset::upset(
  ...
  # Highlight sets using your palette (colors set-size bars + matrix)
  queries = list(
    upset_query(
      set = "Structural",
      fill = unname(fill_vals["Structural"]),
      color = unname(fill_vals["Structural"])
    ),
    upset_query(
      set = "Metabolic",
      fill = unname(fill_vals["Metabolic"]),
      color = unname(fill_vals["Metabolic"])
    )
  )
  
  # (optional) if you had removed set_sizes earlier, you can keep default (fine)
) +
...
# Export
ggsave("Violin_plus_UpSet_final.pdf", p_combo,
